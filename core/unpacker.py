...
class FileUnpacker:
        ...
        try:
            ...
            if hasattr(self.handler, "repack_masterdata"):
                ...
                raise NotImplementedError(
                    f"Saving not implemented for {self.current_file_type} files"
                )
    ...
    def extract_personal_data(self) -> Optional[List[Dict[str, Any]]]:
        """
        Extract PersonalTable data from loaded data.

        Returns:
            List of Pokemon personal data entries or None if not available
        """
        if not self.trainer_data:
            return None

        # Check if we have PersonalTable data
        if "PersonalTable" in self.trainer_data:
            personal_table_data = self.trainer_data["PersonalTable"]
            if (
                isinstance(personal_table_data, dict)
                and "Personal" in personal_table_data
            ):
                return personal_table_data["Personal"]

        return None

    def update_personal_data(self, modified_data: List[Dict[str, Any]]) -> None:
        """
        Update the Personal data in the loaded data.

        Args:
            modified_data: List of modified Pokemon personal entries
        ...
        if "PersonalTable" not in self.trainer_data:
            raise ValueError("No PersonalTable data available")

        self.trainer_data["PersonalTable"]["Personal"] = modified_data
