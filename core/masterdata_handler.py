...
class MasterdataHandler:
    ...
    def save_trainer_data(self, trainer_data: Dict[str, Any]) -> None:
        """
        Save modified trainer data back to the extracted TrainerTable.json.
        
        Args:
            trainer_data: Modified trainer data dictionary
        """
        if not self.trainer_table_path:
            raise RuntimeError("No trainer table file loaded")
        
        with open(self.trainer_table_path, 'wb') as f:
            rapidjson.dump(trainer_data, f, ensure_ascii=False, indent=4)
